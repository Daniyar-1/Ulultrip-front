...
export const jeepCardAction = createAsyncThunk(
    ...
    async (param, {dispatch, rejectWithValue}) => {
        try {
            const reponse = await axios('http://127.0.0.1:8000/api/v1/home/tours/?category=jeep-tur&limit=1&offset=5')
...
export const peshCardAction = createAsyncThunk(
    ...
    async (param, {dispatch, rejectWithValue}) => {
        try {
            const reponse = await axios('http://127.0.0.1:8000/api/v1/home/tours/?category=peshij-tur&limit=1&offset=5')
            if (reponse.status === 200) {
                const data = await reponse.data.results
