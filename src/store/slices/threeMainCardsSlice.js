...
export const leftCardAction = createAsyncThunk(
    ...
    async (param, {dispatch, rejectWithValue}) => {
        try {
            const response = await axios('http://127.0.0.1:8000/api/v1/home/tours/?category=konnyj-tur') //&limit=9
...
export const hotCardAction = createAsyncThunk(
    ...
    async (param, {dispatch, rejectWithValue}) => {
        try {
            const response = await axios('http://127.0.0.1:8000/api/v1/home/tours?limit=50') //?limit=10&offset=10
            if (response.status === 200) {
                const data = await response.data.results
...
export const rightCardAction = createAsyncThunk(
    ...
    async (param, {dispatch, rejectWithValue}) => {
        try {
            const response = await axios('http://127.0.0.1:8000/api/v1/home/tours/?category=velotur')
            // const response = await axios('http://127.0.0.1:8000/api/v1/home/tours/?category=jeep-tur&limit=9')
            if (response.status === 200) {
                const data = await response.data.results
